		...
		ET_PageBuilder.AppView = window.wp.Backbone.View.extend( {
			...
			createLayoutFromContent : function( content, parent_cid, inner_shortcodes ) {
				...
				_.each( matches, function ( shortcode ) {
					...
					if ( shortcode_name.indexOf( 'et_pb_' ) !== -1 || shortcode_name == 'fullwidth_featured_posts') { //  || shortcode_name == 'fullwidth_featured_posts') { // DM - Added condition
...
jQuery(function($){ ET_PageBuilder_Layout = new ET_PageBuilder.Layout; ET_PageBuilder_Layout.get('modules').push({'title':'Featured Post Slider', 'label':'fullwidth_featured_posts', 'fullwidth_only':'on'}); });