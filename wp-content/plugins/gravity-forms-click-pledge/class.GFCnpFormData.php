			...
			switch(RGFormsModel::get_input_type($field)){
				...
				case 'address':
					...
					else
					{
						...
						$parts = array($str1, trim(rgpost("input_{$id}_3")), trim(rgpost("input_{$id}_4")), trim(rgpost("input_{$id}_5")),trim(rgpost("input_{$id}_6")));
						$str2 = implode(', ', array_filter($parts, 'strlen'));
						$item_custom['FieldValue'] = $str2;
						
						//implode(', ', array_filter($str2, 'strlen'));
						$item_custom['FieldId'] = $id;
						$this->customfields[] = $item_custom;
						/*if($item_custom['FieldValue'])
						{
				...
				//	echo $this->ordermode;
					...
					break;
				case 'gfcnpconnectcampaign': 
				
					$this->isConnectCampaignHiddenFlag = RGFormsModel::is_field_hidden($form, $field, RGForms::post('gform_field_values'));
				
					if($this->isConnectCampaignHiddenFlag == FALSE){
					
					$this->connectcampaignField =& $field;
				
					$this->connectcampaign = rgpost('gfcnp_connectcampaign_hid');

				//	echo $this->ordermode;
					$this->connectcampaignCount++;
					}
					break;	
				case 'total':
						...
						switch($field['type'])
						{
							case 'checkbox':								
								...
								for($c = 0; $c < count($inputs); $c++) {
								     $straray = explode(".",$inputs[$c]['id']);
									 $fldid= $straray[1];
									 $val = rgpost("input_{$id}_{$fldid}");
										if($val) {
										...
										$item_custom['FieldName'] = $field["label"] . ': ' . $inputs[$c]['label'];
