...
//print_r($echeck);
	...
	*/
	public function gformFieldInput($input, $field, $value, $lead_id, $form_id) {
		//print_r($field);
		$optionsarry = get_option(gfcnp_plugin);//print_r($optionsarry);

		 $cnpecheckpaymt = $optionsarry['payment_cnp_hidcnpeCheck'];
		
		if ($field['type'] == GFCNP_FIELD_ECHECK || $field['type'] == 'EcheckButton' ) {
			
			if($optionsarry['AccountGuid'] == "" && $cnpecheckpaymt == "" && !IS_ADMIN )
			{
				
						echo "<div style='color: #790000;'>Custom Payment method is not available for your settings.</div>";
				

			}
			elseif ($field['type'] == GFCNP_FIELD_ECHECK) {
...
add_filter( 'gform_admin_pre_render', function ( $form ) {
	...
	$cnpcheck = $optionsarry['payment_cnp_hidcnpeCheck'];
	if(isset($cnpcheck) &&  $cnpcheck == "") {$cnpcheck = "NULL";}
    echo GFCommon::is_form_editor() ? "
        <script type='text/javascript'>
		var jcnpcheck = '".$cnpcheck."';
        gform.addFilter('gform_form_editor_can_field_be_added', function (canFieldBeAdded, type) {
            ...
            if (type == 'gfcnpecheck') {
			 if (GetFieldsByType(['gfcnpecheck']).length > 0) {
				...
				if(jcnpcheck == 'NULL')
				{
					alert('" . __( 'This payment method is not available for your settings', 'gfcnp_plugin' ) . "');
                    return false;
				}
            }
