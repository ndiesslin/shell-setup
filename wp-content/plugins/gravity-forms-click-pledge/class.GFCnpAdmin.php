	...
	*/
	public function __construct($plugin) {
		...
		if(self::is_cnp_page()){
				...
				add_action( 'wp_ajax_cnp_getcode', array('GFCnpAdmin','cnp_getconnectcode') );
				add_action( 'wp_ajax_nopriv_cnp_getcode', array('GFCnpAdmin','cnp_getconnectcode') );
				add_action( 'wp_ajax_cnp_gfgetAccounts', array('GFCnpAdmin','cnp_getcnpaccounts') );
				add_action( 'wp_ajax_nopriv_cnp_gfgetAccounts', array('GFCnpAdmin','cnp_getcnpaccounts') );
				add_action( 'wp_ajax_cnp_refreshAccounts', array('GFCnpAdmin','cnp_refreshAccounts') );
				add_action( 'wp_ajax_nopriv_cnp_refreshAccounts', array('GFCnpAdmin','cnp_refreshAccounts') );
				add_action( 'wp_ajax_cnp_getonchangeAccounts', array('GFCnpAdmin','cnp_getonchangeAccounts') );
				add_action( 'wp_ajax_nopriv_cncnp_getonchangeAccounts', array('GFCnpAdmin','cnp_getonchangeAccounts') );
            }
	}
	...
	public function cnp_getonchangeAccounts()
	{
		$cnpaccountid = $_REQUEST['Accountid'];
		$rtnacntscnpdata =  GFCnpData::getCnPactivePaymentList($cnpaccountid);
		echo $rtnacntscnpdata;
		die();
	}
	public function cnp_refreshAccounts()
	{
		
	$rtnrefreshtokencnpdata =  GFCnpData::getCnPrefreshtoken();
			  ...
			  CURLOPT_HTTPHEADER => array(
				...
				if ($err) {
				  echo "cURL Error #:" . $err;
					...
					$rtncnpdataf = GFCnpData::delete_cnpaccountslist();
					...
					$rtncnpdata = GFCnpData::insert_cnpaccountsinfo($cnporgid,$cnporgname,$cnpaccountid,$cnpufname,$cnplname,$cnpuid);
	...
	public function cnp_getcnpaccounts(){
		
		$cnpemailid = $_REQUEST['cnpemailid'];
		$cnpcode = $_REQUEST['cnpcode'];
		$cnptransactios = GFCnpData::get_cnpgftransactions($cnpemailid,$cnpcode);
		  $curl = curl_init();
		  curl_setopt_array($curl, array(
		  CURLOPT_URL => "https://aaas.cloud.clickandpledge.com/idserver/connect/token",
		  CURLOPT_RETURNTRANSFER => true,
		...
		$cnptokendata = json_decode($response);
		 ...
		 if(!isset($cnptokendata->error)){
			...
			$cnptransactios = GFCnpData::delete_cnptransactions();
			$rtncnpdata =	GFCnpData::insrt_cnptokeninfo($cnpemailid,$cnpcode,$cnptoken,$cnprtoken);	
			  ...
			  CURLOPT_HTTPHEADER => array(
				...
				if ($err) {
				  echo "cURL Error #:" . $err;
					...
					$cnptransactios = GFCnpData::delete_cnpaccountslist();
					...
					foreach($cnpAccountsdata as $cnpkey =>$cnpvalue)
					{
					 ...
					 $cnplname = addslashes($cnpvalue->UserLastName);
				     $cnpuid = $cnpvalue->UserId;
					$cnptransactios = GFCnpData::insert_cnpaccountsinfo($cnporgid,$cnporgname,$cnpaccountid,$cnpufname,$cnplname,$cnpuid);	
	...
	public function cnp_getconnectcode(){
		$curl = curl_init();
	...
	*/
    ...
    private static function edit_page(){
	 ...
	 <div class="wrap">
		  ...
		  <h1><img alt="<?php _e("Click & Pledge Transactions", "gfcnp") ?>" height="50" src="<?php echo self::get_base_url()?>/images/cnp logo.png" style="padding-right: 12px;vertical-align: middle;"/><?php _e("Click & Pledge Settings", "gravityformsauthorizenet") ?></h1>
	...
	private static function list_page(){
		...
		<div class="wrap">
			...
			<h1><img alt="<?php _e("Click & Pledge Transactions", "gfcnp") ?>" height="50" src="<?php echo self::get_base_url()?>/images/cnp logo.png" style="padding-right: 12px;vertical-align: middle;"/> <?php  _e("Click & Pledge Forms", "gfcnp"); ?>
                <a class="button add-new-h2" style="vertical-align:unset;" href="admin.php?page=gfcnp&view=edit&id=0"><?php _e("Add New", "gfcnp") ?></a>
            </h1>
