	...
	*/
	public function __construct() {
		if (self::isFormPost()) {
			...
			$this->payment_cnp_hidcnpcreditcard = self::getPostValue('payment_cnp_hidcnpcreditcard');
			$this->payment_cnp_hidcnpeCheck = self::getPostValue('payment_cnp_hidcnpeCheck');
			$this->payment_cnp_purchas_order = self::getPostValue('payment_cnp_purchas_order');
	...
	public function process() {
		...
		if ($this->frm->isFormPost()) {
			...
			if (empty($errmsg)) {
				...
				$available_cards = array();
				if(isset($_POST['payment_cnp_hidcnpVisa']))
				$available_cards['Visa'] = 'Visa';
				if(isset($_POST['payment_cnp_hidcnpamex']))
				$available_cards['American_Express'] = 'American Express';
				if(isset($_POST['payment_cnp_hidcnpDiscover']))
				$available_cards['Discover'] = 'Discover';
				if(isset($_POST['payment_cnp_hidcnpMaster']))
				$available_cards['MasterCard'] = 'MasterCard';
				if(isset($_POST['payment_cnp_hidcnpjcb']))
				$available_cards['JCB'] = 'JCB';
			...
			$this->plugin->options['payment_cnp_hidcnpcreditcard'] = $this->frm->payment_cnp_hidcnpcreditcard;
			$this->plugin->options['payment_cnp_hidcnpeCheck'] = $this->frm->payment_cnp_hidcnpeCheck;
			$this->plugin->options['payment_cnp_purchas_order'] = $this->frm->payment_cnp_purchas_order;
			...
			$this->frm->payment_cnp_hidcnpcreditcard = $this->plugin->options['payment_cnp_hidcnpcreditcard'];
			$this->frm->payment_cnp_hidcnpeCheck = $this->plugin->options['payment_cnp_hidcnpeCheck'];
			$this->frm->payment_cnp_purchas_order = $this->plugin->options['payment_cnp_purchas_order'];
